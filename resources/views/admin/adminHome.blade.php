  ...
  <body>
            ...
            <div class="row">
              <div class="col-md-4 col-sm-6">
                <div class="statistic-block block">
                  <div class="progress-details d-flex align-items-end justify-content-between">
                    ...
                    <div class="number dashtext-1">{{ $totalUsers -1 }}</div>
                  </div>
              ...
              <div class="col-md-4 col-sm-6">
                <div class="statistic-block block">
                  <div class="progress-details d-flex align-items-end justify-content-between">
                    <div class="title">
                      <div class="icon"><i class="icon-contract"></i></div><strong> Booking</strong>
                    </div>
              ...
              <div class="col-md-4 col-sm-6">
                <div class="statistic-block block">
                  <div class="progress-details d-flex align-items-end justify-content-between">
                    <div class="title">
                      <div class="icon"><i class="icon-paper-and-pencil"></i></div><strong>Total Price</strong>
                    </div>
                    <div class="number dashtext-3">{{$totalPrices}}</div>
                  </div>
                  <div class="progress progress-template">
    ...
    .then(data => {
        ...
        const revenueChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: data.months, // Tháng
                datasets: [{
                    label: 'Doanh Thu (VND)',
                    data: data.revenues, // Doanh thu
    ...
    .catch(error => console.error('Error fetching revenue data:', error));
        ...
        <script>
                ...
                fetch('/annual-revenue')
                    ...
                    .catch(error => console.error('Error fetching annual revenue data:', error));

                // Lấy danh sách năm để tạo dropdown
                fetch('/annual-revenue')
                    .then(response => response.json())
                    .then(data => {
                        ...
                        // Tự động hiển thị doanh thu của năm đầu tiên khi trang được tải
                        yearSelect.value = data.years[0];
                        fetchMonthlyRevenue(data.years[0]);
                    })
                    .catch(error => console.error('Error fetching year data:', error));

                // Hàm để lấy doanh thu theo tháng khi chọn năm
                function fetchMonthlyRevenue(year) {
                    fetch(`/monthly-revenue?year=${year}`)
                        .then(response => response.json())
                        .then(data => {
                            ...
                            const monthlyRevenueChart = new Chart(ctx, {
                                type: 'bar',
                                data: {
                                    labels: data.months.map(month => month.toString().padStart(2, '0')), // Tháng
                                    datasets: [{
                                        label: 'Doanh Thu (VND)',
                                        data: data.revenues, // Doanh thu
                ...
                // Lắng nghe sự kiện khi người dùng chọn năm khác
                document.getElementById('yearSelect').addEventListener('change', function() {
                    fetchMonthlyRevenue(this.value);
                });
            </script>
        <!-- <section class="no-padding-bottom">
          <div class="container-fluid">
